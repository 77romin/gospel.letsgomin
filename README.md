...
> 현재 공개 화면은 **개인 연습 전용**입니다. 여러 기기의 재생을 맞추는 「같이 연습」 기능은 모바일에서 충분한 동기화 품질을 확인하지 못해 잠시 숨겼습니다. 지휘자 로그인과 연습 구간 관리는 사용할 수 있습니다.
...
| 영상 보기 | 영상이 포함된 음원에서 `Show Video`로 현재 위치의 영상을 열고 닫기 |
...
4. 영상이 있는 음원은 `Show Video`를 눌러 악보·영상을 함께 볼 수 있습니다. 영상을 닫아도 같은 음원이 이어집니다.
...
이 테스트는 로컬 Node 서버의 권한, 구간 변경, 음원 제공 및 공동 재생 상태를 확인합니다. 현재 공개 화면에서는 공동 재생 UI가 숨겨져 있습니다.

## 기술 구성과 배포

| 영역 | 구성 |
| --- | --- |
| 화면·미디어 | Vanilla JavaScript, HTML, CSS, MP4 영상·음성 |
...
곡을 바꿀 때는 [`public/audio`](public/audio)에 다섯 파트 파일을 넣고 [`public/tracks.json`](public/tracks.json)의 곡 ID·파일 경로·길이를 수정합니다. 파트 간 시작 지점과 길이를 맞춰야 합니다. 운영 데이터베이스의 곡 ID와 길이도 함께 맞춰야 하며, 자세한 주의사항은 배포 문서를 참고하세요.
