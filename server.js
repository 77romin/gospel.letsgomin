...
for (const track of partNames.map(part => catalog.tracks[part])) {
  if (!track || typeof track.file !== 'string' || !/^\/audio\/[a-zA-Z0-9._-]+\.(mp3|m4a|wav|ogg|webm)$/.test(track.file)) throw new Error('음원 경로는 /audio/ 아래 파일이어야 합니다.');
...
function mime(file) {
  return ({ '.html': 'text/html', '.css': 'text/css', '.js': 'text/javascript', '.svg': 'image/svg+xml', '.mp3': 'audio/mpeg', '.m4a': 'audio/mp4', '.wav': 'audio/wav', '.ogg': 'audio/ogg', '.webm': 'audio/webm' })[path.extname(file)] || 'application/octet-stream';
}
