...
재생뿐 아니라 정지도 `현재 서버 시각 + 3000ms`로 예약한다. 네트워크가 빠른 기기와 느린 기기 모두 명령을 받은 뒤 같은 목표 시각을 기다린다. 재생 중 seek도 새 미래 시각으로 다시 예약한다.
...
- 자동 테스트에서 서버 시각 변환, 성능 시각-오디오 시각 변환, 3초 재생·정지 예약, 권한과 상태 전파를 검증했다.
- Vite 프로덕션 빌드를 통과했다.
...
- 현재 3초 준비 시간은 단순하고 보수적인 고정값이다. 참여자가 많아지거나 네트워크 품질 차이가 커지면 BeatSync처럼 최대 RTT를 반영한 동적 예약 시간으로 발전시킬 수 있다.
